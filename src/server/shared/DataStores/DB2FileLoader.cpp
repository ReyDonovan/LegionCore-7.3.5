...
DB2FileLoadInfo::DB2FileLoadInfo(DB2FieldMeta const* fields, std::size_t fieldCount, DB2Meta const* meta) : Fields(fields), FieldCount(fieldCount), Meta(meta)
{
...
uint32 DB2FileLoadInfo::GetStringFieldCount(bool localizedOnly) const
{
    ...
    for (auto fieldType : TypesString)
...
public:
    ...
    ~DB2FileLoaderRegularImpl() override;
...
public:
    ...
    ~DB2FileLoaderSparseImpl() override;
...
DB2FileLoaderRegularImpl::DB2FileLoaderRegularImpl(char const* fileName, DB2FileLoadInfo const* loadInfo, DB2Header const* header) : _fileName(fileName), _loadInfo(loadInfo), _header(header), _stringTable(nullptr)
{
}
...
char* DB2FileLoaderRegularImpl::AutoProduceData(uint32& records, char**& indexTable, std::vector<char*>& stringPool)
{
    ...
    if (stringFields)
    {
        ...
        for (std::size_t i = 0; i < stringHoldersPoolSize / sizeof(char*); ++i)
            reinterpret_cast<char const**>(stringHoldersPool)[i] = nullStr;
    }
    ...
    for (uint32 y = 0; y < _header->RecordCount; y++)
    {
            ...
            for (uint32 z = 0; z < _loadInfo->Meta->Fields[x].ArraySize; ++z)
            {
                switch (_loadInfo->TypesString[fieldIndex])
                {
                    ...
                    case FT_STRING_NOT_LOCALIZED:
                    {
                        ...
                        auto slot = reinterpret_cast<char const***>(&dataTable[offset]);
                        *slot = reinterpret_cast<char const**>(&stringHoldersPool[stringHoldersRecordPoolSize * y + stringFieldOffset]);
        ...
        for (uint32 x = _header->FieldCount; x < _loadInfo->Meta->FieldCount; ++x)
        {
            for (uint32 z = 0; z < _loadInfo->Meta->Fields[x].ArraySize; ++z)
            {
                ...
                {
                    case FT_FLOAT:
                        *reinterpret_cast<float*>(&dataTable[offset]) = 0;
                        offset += 4;
                        break;
                    case FT_INT:
                    ...
                    case FT_SHORT:
                        *reinterpret_cast<uint16*>(&dataTable[offset]) = 0;
                        ...
                        break;
                    case FT_LONG:
                        *reinterpret_cast<uint64*>(&dataTable[offset]) = 0;
                        offset += 8;
                        break;
                    case FT_STRING:
                    case FT_STRING_NOT_LOCALIZED:
...
char* DB2FileLoaderRegularImpl::AutoProduceStrings(char** indexTable, uint32 indexTableSize, uint32 locale)
{
    ...
    auto stringPool = new char[_header->StringTableSize];
...
char const* DB2FileLoaderRegularImpl::RecordGetString(uint8 const* record, uint32 field, uint32 arrayIndex) const
{
    ...
    auto stringOffset = RecordGetVarInt<uint32>(record, field, arrayIndex);
...
T DB2FileLoaderRegularImpl::RecordGetVarInt(uint8 const* record, uint32 field, uint32 arrayIndex) const
{
    ...
    switch (compressionType)
    {
        ...
        {
            ASSERT(arrayIndex == 0);
            auto immediateValue = RecordGetPackedValue(record + GetFieldOffset(field), _columnMeta[field].CompressionData.immediate.BitWidth, _columnMeta[field].CompressionData.immediate.BitOffset);
        ...
        {
            ASSERT(arrayIndex == 0);
            auto palletIndex = RecordGetPackedValue(record + GetFieldOffset(field), _columnMeta[field].CompressionData.pallet.BitWidth, _columnMeta[field].CompressionData.pallet.BitOffset);
            EndianConvert(palletIndex);
            auto palletValue = _palletValues[field][palletIndex].Value;
        ...
        case DB2ColumnCompression::PalletArray:
        {
            auto palletIndex = RecordGetPackedValue(record + GetFieldOffset(field), _columnMeta[field].CompressionData.pallet.BitWidth, _columnMeta[field].CompressionData.pallet.BitOffset);
            EndianConvert(palletIndex);
            auto palletValue = _palletArrayValues[field][palletIndex * _columnMeta[field].CompressionData.pallet.ArraySize + arrayIndex].Value;
            EndianConvert(palletValue);
            T value;
            memcpy(&value, &palletValue, std::min(sizeof(T), sizeof(palletValue)));
            return value;
...
DB2FileLoaderSparseImpl::DB2FileLoaderSparseImpl(char const* fileName, DB2FileLoadInfo const* loadInfo, DB2Header const* header) : _fileName(fileName), _loadInfo(loadInfo), _header(header), _dataStart(0), _fieldAndArrayOffsets(Trinity::make_unique<std::size_t[]>(loadInfo->Meta->FieldCount + loadInfo->FieldCount - (!loadInfo->Meta->HasIndexFieldInData() ? 1 : 0))) { }
...
char* DB2FileLoaderSparseImpl::AutoProduceData(uint32& maxId, char**& indexTable, std::vector<char*>& stringPool)
{
    ...
    auto dataTable = new char[records * recordsize];
    ...
    // each string field at load have array of string for each locale
    auto stringHoldersRecordPoolSize = localizedStringFields * sizeof(LocalizedString) + (stringFields - localizedStringFields) * sizeof(char*);
    auto stringHoldersPoolSize = stringHoldersRecordPoolSize * records;

    auto stringHoldersPool = new char[stringHoldersPoolSize];
    stringPool.push_back(stringHoldersPool);
    ...
    auto stringTable = new char[expandedDataSize - records * ((recordsize - (!_loadInfo->Meta->HasIndexFieldInData() ? 4 : 0)) - stringFields * sizeof(char*))];
    ...
    auto stringPtr = stringTable;

    uint32 offset = 0;
    uint32 recordNum = 0;
    for (uint32 y = 0; y < offsetCount; ++y)
    {
        auto rawRecord = GetRawRecordData(y);
        if (!rawRecord)
            continue;

        auto indexVal = RecordGetId(rawRecord, y);
        indexTable[indexVal] = &dataTable[offset];
        ...
        for (uint32 x = 0; x < _header->FieldCount; ++x)
        {
            for (uint32 z = 0; z < _loadInfo->Meta->Fields[x].ArraySize; ++z)
            {
                switch (_loadInfo->TypesString[fieldIndex])
                {
                    ...
                    case FT_STRING:
                    {
                        auto slot = reinterpret_cast<LocalizedString**>(&dataTable[offset]);
                        *slot = reinterpret_cast<LocalizedString*>(&stringHoldersPool[stringHoldersRecordPoolSize * recordNum + stringFieldOffset]);
                    ...
                    {
                        auto slot = reinterpret_cast<char const***>(&dataTable[offset]);
                        *slot = reinterpret_cast<char const**>(&stringHoldersPool[stringHoldersRecordPoolSize * recordNum + stringFieldOffset]);
                        **slot = stringPtr;
                        strcpy(stringPtr, RecordGetString(rawRecord, x, z));
                        stringPtr += strlen(stringPtr) + 1;
                        stringFieldOffset += sizeof(char*);
...
char* DB2FileLoaderSparseImpl::AutoProduceStrings(char** indexTable, uint32 indexTableSize, uint32 locale)
{
    ...
    auto offsetCount = _header->MaxId - _header->MinId + 1;
    uint32 records = 0;
    for (uint32 i = 0; i < offsetCount; ++i)
        if (_catalog[i].FileOffset && _catalog[i].RecordSize)
            ++records;

    auto recordsize = _loadInfo->Meta->GetRecordSize();
    ...
    for (uint32 y = 0; y < offsetCount; y++)
    {
        ...
        auto recordData = indexTable[indexVal];
...
std::size_t* DB2FileLoaderSparseImpl::RecordCreateDetachedFieldOffsets(std::size_t* oldOffsets) const
{
    ...
    auto size = _loadInfo->Meta->FieldCount + _loadInfo->FieldCount - (!_loadInfo->Meta->HasIndexFieldInData() ? 1 : 0);
    auto newOffsets = new std::size_t[size];
...
DB2Record::DB2Record(DB2FileLoaderImpl const& db2, uint32 recordIndex, std::size_t* fieldOffsets) : _db2(db2), _recordIndex(recordIndex), _recordData(db2.GetRawRecordData(recordIndex)), _fieldOffsets(fieldOffsets)
{
}
...
bool DB2FileLoader::Load(DB2FileSource* source, DB2FileLoadInfo const* loadInfo)
{
    ...
    {
        auto expectedFileSize = sizeof(DB2Header) + sizeof(DB2SectionHeader) * _header.SectionCount + sizeof(DB2FieldEntry) * _header.FieldCount + _header.RecordCount * _header.RecordSize + _header.StringTableSize + (loadInfo->Meta->IndexField == -1 ? _header.RecordCount * sizeof(uint32) : 0) + totalCopyTableSize + _header.ColumnMetaSize + _header.PalletDataSize + _header.CommonDataSize + totalParentLookupDataSize;
        if (source->GetFileSize() != expectedFileSize)
            return false;
    }

    auto fieldData = Trinity::make_unique<DB2FieldEntry[]>(_header.FieldCount);
    ...
    if (_header.ColumnMetaSize)
    {
        ...
        ASSERT(!loadInfo->Meta->HasIndexFieldInData() || columnMeta[loadInfo->Meta->IndexField].CompressionType == DB2ColumnCompression::None || columnMeta[loadInfo->Meta->IndexField].CompressionType == DB2ColumnCompression::Immediate || columnMeta[loadInfo->Meta->IndexField].CompressionType == DB2ColumnCompression::SignedImmediate);
        ...
        auto commonData = Trinity::make_unique<std::unique_ptr<DB2CommonValue[]>[]>(_header.TotalFieldCount);
