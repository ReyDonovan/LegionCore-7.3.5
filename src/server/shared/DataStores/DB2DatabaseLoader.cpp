...
DB2LoadInfo::DB2LoadInfo(DB2FieldMeta const* fields, std::size_t fieldCount, DB2Meta const* meta, HotfixDatabaseStatements statement) : DB2FileLoadInfo(fields, fieldCount, meta), Statement(statement)
{
}
...
char* DB2DatabaseLoader::Load(uint32& records, char**& indexTable, char*& stringHolders, std::vector<char*>& stringPool)
{
    ...
    if (stringFields)
    {
        ...
        for (std::size_t i = 0; i < stringHoldersPoolSize / sizeof(char*); ++i)
            reinterpret_cast<char const**>(stringHolders)[i] = nullStr;
    }
    else
    ...
    do
    {
        ...
        {
            *reinterpret_cast<uint32*>(&dataValue[offset]) = indexValue;
            offset += 4;
            ++f;
        }
        ...
        for (uint32 x = 0; x < _loadInfo->Meta->FieldCount; ++x)
        {
            for (uint32 z = 0; z < _loadInfo->Meta->Fields[x].ArraySize; ++z)
            {
                ...
                {
                    case FT_FLOAT:
                        *reinterpret_cast<float*>(&dataValue[offset]) = fields[f].GetFloat();
                        offset += 4;
                        break;
                    case FT_INT:
                        *reinterpret_cast<int32*>(&dataValue[offset]) = fields[f].GetInt32();
                        offset += 4;
                        break;
                    case FT_BYTE:
                        *reinterpret_cast<int8*>(&dataValue[offset]) = fields[f].GetInt8();
                        offset += 1;
                        break;
                    case FT_SHORT:
                        *reinterpret_cast<int16*>(&dataValue[offset]) = fields[f].GetInt16();
                        offset += 2;
                        break;
                    case FT_LONG:
                        *reinterpret_cast<int64*>(&dataValue[offset]) = fields[f].GetInt64();
                        offset += 8;
                        break;
                    case FT_STRING:
                    {
                        LocalizedString** slot = reinterpret_cast<LocalizedString**>(&dataValue[offset]);
                        *slot = reinterpret_cast<LocalizedString*>(&stringHolders[stringHoldersRecordPoolSize * rec + stringFieldOffset]);
                        ASSERT(*slot);
                    ...
                    {
                        char const** slot = reinterpret_cast<char const**>(&dataValue[offset]);
                        *slot = static_cast<char*>(&stringHolders[stringHoldersRecordPoolSize * rec + stringFieldOffset]);
                        ASSERT(*slot);
                ...
                for (uint32 z = 0; z < _loadInfo->Meta->Fields[x].ArraySize; ++z)
                {
                    switch (_loadInfo->TypesString[fieldIndex])
                    {
                        ...
                        case FT_STRING:
                        {
                            ...
                            LocalizedString* db2str = *reinterpret_cast<LocalizedString**>(&dataValue[offset]);
